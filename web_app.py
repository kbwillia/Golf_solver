...
def start_game():
    ...
    # Store the actual game object data
    session['game_data'] = {
        'num_players': num_players,
        'agent_types': agent_types,
...
def take_action():
    ...
    # Restore game state
    game.deck = []
    for card_str in game_data['deck']:
        if card_str:
            rank, suit = card_str[0], card_str[1]
            game.deck.append(Card(rank, suit))

    game.discard_pile = []
    for card_str in game_data['discard_pile']:
        if card_str:
            rank, suit = card_str[0], card_str[1]
            game.discard_pile.append(Card(rank, suit))
    ...
    for i, player in enumerate(game.players):
        ...
        player.grid = []
        for card_str in player_state['grid']:
            if card_str:
                rank, suit = card_str[0], card_str[1]
                player.grid.append(Card(rank, suit))
            else:
                player.grid.append(None)
